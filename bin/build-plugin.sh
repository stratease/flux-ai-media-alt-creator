...
# Build script for Flux AI Media Alt Creator WordPress plugin
...
PLUGIN_NAME="flux-ai-media-alt-creator"
PLUGIN_FILE="$PLUGIN_DIR/flux-ai-media-alt-creator.php"
...
extract_php_constant_version() {
    if [ -f "$PLUGIN_FILE" ]; then
        ...
        grep "FLUX_AI_MEDIA_ALT_CREATOR_VERSION" "$PLUGIN_FILE" | sed -n "s/.*'FLUX_AI_MEDIA_ALT_CREATOR_VERSION',[[:space:]]*'\([^']*\)'.*/\1/p" | head -1
    fi
}
...
if [ -z "$NEW_VERSION" ]; then
    ...
    if [ -f "$PLUGIN_FILE" ]; then
        if [[ "$OSTYPE" == "darwin"* ]]; then
            ...
            sed -i '' "s/define( 'FLUX_AI_MEDIA_ALT_CREATOR_VERSION', '[^']*' );/define( 'FLUX_AI_MEDIA_ALT_CREATOR_VERSION', '$NEW_VERSION' );/" "$PLUGIN_FILE"
        else
...
TEMP_BUILD_DIR="/tmp/flux-ai-media-alt-creator-build-$$"
